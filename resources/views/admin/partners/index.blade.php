...
@section('page_subtitle', 'Buat dan atur partner Anda di sini.')

@section('content')

<div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2">
    <form method="GET" action="{{ route('admin.partners.index') }}" class="flex-1 flex items-center gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Partner..." class="w-full px-5 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" />
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y border-t">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <img src="{{ $partner->logo_url }}" alt="Logo {{ $partner->name }}" class="w-16 h-16 object-contain rounded-xl border border-slate-100 bg-slate-50">
                    </td>
                    <td class="px-8 py-6 font-semibold">{{ $partner->name }}</td>
                    <td class="px-8 py-6">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-8 py-8 text-center text-slate-400">Belum ada data partner.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
